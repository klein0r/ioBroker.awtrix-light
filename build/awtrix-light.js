...
var awtrix_light_exports = {};
__export(awtrix_light_exports, {
  AwtrixLight: () => AwtrixLight
});
module.exports = __toCommonJS(awtrix_light_exports);
...
class AwtrixLight extends utils.Adapter {
    ...
    if (id && state && !state.ack) {
      ...
      if (this.apiClient.isConnected()) {
        if (idNoNamespace.startsWith("settings.")) {
          ...
          this.apiClient.requestAsync("nextapp", "POST").then(async (response) => {
          ...
          this.apiClient.requestAsync("previousapp", "POST").then(async (response) => {
            if (response.status === 200) {
              await this.setState(idNoNamespace, { val: state.val, ack: true });
            }
          }).catch((error) => {
  ...
  async updateIndicatorByStates(index) {
    ...
    return this.apiClient.indicatorRequestAsync(index, void 0);
  }
  async updateMoodlightByStates() {
    ...
    return this.apiClient.requestAsync("moodlight", "POST", void 0);
  }
  removeNamespace(id) {
...
//# sourceMappingURL=awtrix-light.js.map
